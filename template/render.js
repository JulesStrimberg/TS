 ...
 *
 * Entrée : un objet `config` = un élément du tableau `clienti` du fichier de données.
 */
(function (root, factory) {
  ...
  function renderFooter(config) {
    ...
    /* La partita IVA du commerçant doit figurer sur le site d'une activité
       commerciale italienne. C'est la sienne, jamais celle du prestataire. */
    var legale = '© <span data-anno>' + new Date().getFullYear() + '</span> ' + esc(config.nome);
    if (ok(config.partita_iva)) {
      legale += ' · P. IVA ' + esc(String(config.partita_iva).replace(/\s+/g, ''));
    }
    return '<footer class="footer">' +
      '<div class="footer__interno">' +
      '<p class="footer__nome">' + esc(config.nome) + '</p>' +
      '<address class="footer__contatti">' + righe.join('') + '</address>' +
      '<p class="footer__legale">' + legale + '</p>' +
      '</div></footer>';
  }
  ...
  function jsonLd(config) {
    ...
    if (ok(config.partita_iva)) dati.vatID = String(config.partita_iva).replace(/\s+/g, '');
    if (ok(config.indirizzo)) {
