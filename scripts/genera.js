...
function leggiArgomenti(argv) {
  const opz = { dati: 'data/clienti.json', out: 'siti', solo: null, pulisci: false, mapsKey: process.env.GOOGLE_MAPS_EMBED_KEY || '' };
...
Uso : node scripts/genera.js [opzioni]
  ...
  --dati <file>     JSON dei clienti          (default: data/clienti.json)
...
function main() {
  ...
  const dati = JSON.parse(fs.readFileSync(path.resolve(ROOT, opz.dati), 'utf8'));
